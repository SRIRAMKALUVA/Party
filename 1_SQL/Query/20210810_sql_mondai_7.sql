SELECT 
    ...
    count(*) AS 参加者総数, coalesce(sum(CASE WHEN gender_kbn = '00101' THEN price_man END),0) AS 男性売上げ,
    coalesce(sum(CASE WHEN gender_kbn = '00102' THEN price_woman END),0) AS 女性売上げ,
    coalesce(sum(CASE WHEN gender_kbn = '00101' THEN price_man END),0) + coalesce(sum(CASE WHEN gender_kbn = '00102' THEN price_woman END),0) AS 総売上げ
